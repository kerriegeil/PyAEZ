...
class ETOCalc(object):
    ...
    def calculateETO(self):
        ...
        ndays=len(doy)
        sd = 0.4093 * np.sin((2*np.pi*doy/ndays)-1.39)
        ...
        lat_rad = np.tile(lat_rad[:,:,np.newaxis],(1,1,ndays))

        # Sunset Hour angle
        sha = np.arccos(-np.tan(lat_rad)*np.tan(sd))

        # Relative distance Earth to Sun
        dr = 1 + 0.033*np.cos(2*np.pi*doy/ndays)
        ...
        del sd, lat_rad, sha, dr
        
        # Clear-sky solar radiation
        alt=np.tile(self.alt[:,:,np.newaxis],(1,1,ndays))
