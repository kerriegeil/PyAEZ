...
import UtilitiesCalc as UtilitiesCalc #KLG
...
class ClimateRegime(object):
            ...
            for i_col in range(self.im_width):
                ...
                # SubTropic
                    ...
                    if np.sum(self.totalPrec_daily[i_row,i_col,:]) < 250:
                        ...
                        if self.latitude[i_row,i_col]>=0: 
                            if summer_PET0 >= winter_PET0:
                                # Subtropics Summer Rainfall
                                thermal_climate[i_row,i_col] = 3
                            else:
    ...
    def getLGP(self, Sa=100., D=1.):
        ...
        # Sb_old = 0. # needs to go inside the spatial loops (it's initialization, not a constant)
        # Wb_old = 0. # needs to go inside the spatial loops (it's initialization, not a constant)
        #============================ 
        ...
        # istart0_save = np.empty((self.im_height, self.im_width))
        # istart1_save = np.empty((self.im_height, self.im_width))
        # p_save = np.empty(Tx365.shape)
        # xx_save = np.empty(Tx365.shape)
        # yy_save = np.empty(Tx365.shape)
        # islgp_save = np.empty(Tx365.shape)
        self.eta_class = np.zeros(Tx365.shape)
        self.snm365 = np.zeros(Tx365.shape)
        ...
        for i_row in range(self.im_height):
            for i_col in range(self.im_width):
                ...
                # istart0_save[i_row,i_col]=istart0
                # istart1_save[i_row,i_col]=istart1
                ### update!!! ###
                ### put sb_old, wb_old inside spatial loops else they won't be initialized correctly ###
                Sb_old = 0.
                Wb_old = 0.
                #----------------------------------
                if self.set_mask:
                ...
                for doy in range(0, 365):
                    ...
                    # p_save[i_row, i_col, doy]=p

                    Eta_new, Etm_new, Wb_new, Wx_new, Sb_new, kc_new, cat_new,snm = LGPCalc.EtaCalc(
                    ...
                    self.eta_class[i_row, i_col, doy]=cat_new
                    self.snm365[i_row, i_col, doy]=snm

                    Wb_old = Wb_new
                    Sb_old = Sb_new
        #============================================
        # self.istart0=istart0_save
        # self.istart1=istart1_save
        # self.p=p_save
        for i_row in range(self.im_height):
            for i_col in range(self.im_width):
                if self.set_mask:
                ...
                # xx_save[i_row,i_col,:]=xx[:365]
                # yy_save[i_row,i_col,:]=yy[:365]
                # islgp_save[i_row,i_col,:]=islgp
            ...
            for i_c in range(self.im_width):
                if self.set_mask:
                    if self.im_mask[i_r, i_c] == self.nodata_val:
                        ...
                        ## note about this next line, the condition with an 'in' will not work correctly if not
                        ## enclosed in parentheses! But it also won't throw an error. 
    ...
    def getMultiCroppingZones(self, t_climate, lgp, lgp_t5, lgp_t10, ts_t0, ts_t10):
        # order of the inputs between the notebook and this code is backward
        # for the last 2 variables
