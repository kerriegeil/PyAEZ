...
#         P_by_PET_ONDJFM=P_by_PET_monthly[:,:,JFMSON].mean(axis=2) # Oct-Mar mean
        summer_PET0=P_by_PET_monthly[:,:,3:9].sum(axis=2)
        winter_PET0=P_by_PET_monthly[:,:,JFMSON].sum(axis=2)
...
#         range_meanT=meanT_monthly.max(axis=2) - meanT_monthly.min(axis=2)
        ...
        # Tropical highland
        thermal_climate=np.where((min_sealev_meanT>=18) & (Ta_diff<15) & (meanT<=20) & ~np.isfinite(thermal_climate),2,thermal_climate)
        # SubTropic
        ...
        thermal_climate=np.where((min_sealev_meanT>=5) & (nmo_ge_10C>=8) & (self.latitude>=0) & (summer_PET0>=winter_PET0) & ~np.isfinite(thermal_climate),4,thermal_climate)
        thermal_climate=np.where((min_sealev_meanT>=5) & (nmo_ge_10C>=8) & (self.latitude<0) & (summer_PET0<=winter_PET0) & ~np.isfinite(thermal_climate),4,thermal_climate)
        # Subtropics Winter Rainfall
        thermal_climate=np.where((min_sealev_meanT>=5) & (nmo_ge_10C>=8) & (self.latitude>=0) & (summer_PET0<=winter_PET0) & ~np.isfinite(thermal_climate),5,thermal_climate)
        thermal_climate=np.where((min_sealev_meanT>=5) & (nmo_ge_10C>=8) & (self.latitude<0) & (summer_PET0>=winter_PET0) & ~np.isfinite(thermal_climate),5,thermal_climate)
        # Temperate
        # Oceanic Temperate
        thermal_climate=np.where((nmo_ge_10C>=4) & (Ta_diff<=20) & ~np.isfinite(thermal_climate),6,thermal_climate)
        # Sub-Continental Temperate
        thermal_climate=np.where((nmo_ge_10C>=4) & (Ta_diff<=35) & ~np.isfinite(thermal_climate),7,thermal_climate)
        # Continental Temperate
        thermal_climate=np.where((nmo_ge_10C>=4) & (Ta_diff>35) & ~np.isfinite(thermal_climate),8,thermal_climate)
        # Boreal
        # Oceanic Boreal
        thermal_climate=np.where((nmo_ge_10C>=1) & (Ta_diff<=20) & ~np.isfinite(thermal_climate),9,thermal_climate)
        # Sub-Continental Boreal
        thermal_climate=np.where((nmo_ge_10C>=1) & (Ta_diff<=35) & ~np.isfinite(thermal_climate),10,thermal_climate)
        # Continental Boreal
        thermal_climate=np.where((nmo_ge_10C>=1) & (Ta_diff>35) & ~np.isfinite(thermal_climate),11,thermal_climate)
        # Arctic
...
#         multi_crop_rain=np.ones(self.im_mask.shape)    
#         # 8 => H Zone (Zone of triple rice cropping; 
#         # sequential cropping of three wetland rice crops is possible)
#         multi_crop_rain=np.where( (tclimate==1)&(lgp>=360)&(lgp_t5>=360)&(lgp_t10>=360)&\
#                                  (ts_t0>=7200)&(ts_t10>=7000)&\
#                                  (multi_crop_rain==1),8, multi_crop_rain )
#         multi_crop_rain=np.where( (tclimate!=1)&(lgp>=360)&(lgp_t5>=360)&(lgp_t10>=330)&\
#                                  (ts_t0>=7200)&(ts_t10>=7000)\
#                                  &(multi_crop_rain==1),8, multi_crop_rain )
        
#         # 7 => G Zone (Zone of triple cropping; 
#         # sequential cropping of three short-cycle crops; two wetland rice crops are possible)
#         multi_crop_rain=np.where( (tclimate!=1)&(lgp>=330)&(lgp_t5>=330)&(lgp_t10>=270)&\
#                                  (ts_t0>=5700)&(ts_t10>=5500)&\
#                                  (multi_crop_rain==1),7, multi_crop_rain )
        
#         # 6 => F Zone (Zone of double rice cropping or limited triple cropping; 
#         # may partly involve relay cropping; a third crop is not possible in case of two wetland rice crops)
#         multi_crop_rain=np.where( (tclimate==1)&(lgp>=300)&(lgp_t5>=300)&(lgp_t10>=240)&\
#                                  (ts_t0>=7200)&\
#                                  (ts_g_t5>=5100)&(ts_g_t10>=4800)&(multi_crop_rain==1),6, multi_crop_rain )
#         multi_crop_rain=np.where( (tclimate!=1)&(lgp>=300)&(lgp_t5>=300)&(lgp_t10>=240)&\
#                                  (ts_t0>=5400)&(ts_t10>=5100)&\
#                                  (ts_g_t5>=5100)&(ts_g_t10>=4800)&(multi_crop_rain==1),6, multi_crop_rain )
      
#         # 5 => E Zone (Zone of double cropping with rice; 
#         # sequential double cropping with one wetland rice crop is possible)
#         multi_crop_rain=np.where( (tclimate!=1)&(lgp>=240)&(lgp_t5>=270)&(lgp_t10>=180)&\
#                                  (ts_t0>=4800)&(ts_t10>=4500)&\
#                                  (ts_g_t5>=4300)&(ts_g_t10>=4000)&(multi_crop_rain==1),5, multi_crop_rain )      
        
#         # 4 => D Zone (Zone of double cropping; 
#         # sequential double cropping including wetland rice is not possible)
#         multi_crop_rain=np.where( (tclimate==1)&(lgp>=270)&(lgp_t5>=270)&(lgp_t10>=165)&\
#                                  (ts_t0>=5500)&\
#                                  (ts_g_t5>=4000)&(ts_g_t10>=3200)&(multi_crop_rain==1), 4, multi_crop_rain )
#         multi_crop_rain=np.where( (tclimate==1)&(lgp>=240)&(lgp_t5>=240)&(lgp_t10>=165)&\
#                                  (ts_t0>=6400)&\
#                                  (ts_g_t5>=4000)&(ts_g_t10>=3200)&(multi_crop_rain==1), 4, multi_crop_rain )
#         multi_crop_rain=np.where( (tclimate==1)&(lgp>=210)&(lgp_t5>=240)&(lgp_t10>=165)&\
#                                  (ts_t0>=7200)&\
#                                  (ts_g_t5>=4000)&(ts_g_t10>=3200)&(multi_crop_rain==1), 4, multi_crop_rain )
#         multi_crop_rain=np.where( (tclimate!=1)&(lgp>=210)&(lgp_t5>=240)&(lgp_t10>=165)&\
#                                  (ts_t0>=4500)&(ts_t10>=3600)&\
#                                  (ts_g_t5>=4000)&(ts_g_t10>=3200)&(multi_crop_rain==1), 4, multi_crop_rain )             multi_crop_rain=np.where( (tclimate!=1)&(lgp>=210)&(lgp_t5>=240)&(lgp_t10>=165)&\
#                                  (ts_t0>=4500)&(ts_t10>=3600)&\
#                                  (ts_g_t5>=4000)&(ts_g_t10>=3200)&(multi_crop_rain==1), 4, multi_crop_rain ) 

#         # 3 => C Zone (Zone of limited double cropping, relay cropping; single wetland rice may be possible)
#         multi_crop_rain=np.where( (tclimate==1)&(lgp>=220)&(lgp_t5>=220)&(lgp_t10>=120)&\
#                                  (ts_t0>=5500)&\
#                                  (ts_g_t5>=3200)&(ts_g_t10>=2700)&(multi_crop_rain==1), 3, multi_crop_rain )
#         multi_crop_rain=np.where( (tclimate==1)&(lgp>=200)&(lgp_t5>=200)&(lgp_t10>=120)&\
#                                  (ts_t0>=6400)&\
#                                  (ts_g_t5>=3200)&(ts_g_t10>=2700)&(multi_crop_rain==1), 3, multi_crop_rain )
#         multi_crop_rain=np.where( (tclimate==1)&(lgp>=180)&(lgp_t5>=200)&(lgp_t10>=120)&\
#                                  (ts_t0>=7200)&\
#                                  (ts_g_t5>=3200)&(ts_g_t10>=2700)&(multi_crop_rain==1), 3, multi_crop_rain )
#         multi_crop_rain=np.where( (tclimate!=1)&(lgp>=180)&(lgp_t5>=200)&(lgp_t10>=120)&\
#                                  (ts_t0>=3600)&(ts_t10>=3000)&\
#                                  (ts_g_t5>=3200)&(ts_g_t10>=2700)&(multi_crop_rain==1), 3, multi_crop_rain )                    
                    
#         # 2 => B Zone (Zone of single cropping)
#         multi_crop_rain=np.where( (tclimate==1)&(lgp>=45)&(lgp_t5>=120)&(lgp_t10>=90)&\
#                                  (ts_t0>=1600)&(ts_t10>=1200)&\
#                                  (multi_crop_rain==1), 2, multi_crop_rain )
#         multi_crop_rain=np.where( (tclimate!=1)&(lgp>=45)&(lgp_t5>=120)&(lgp_t10>=90)&\
#                                  (ts_t0>=1600)&(ts_t10>=1200)&\
#                                  (multi_crop_rain==1), 2, multi_crop_rain )        
#         # 1 => A Zone (Zone of no cropping, too cold or too dry for rainfed crops)
#         # everything else equals 1 because we initialized the array to one       
...
#         # copy the remaining updates to here
...
#             return [multi_crop_rain, multi_crop_irr]
    ...
    def AirFrostIndexandPermafrostEvaluation(self):
        ...
        permafrost=permafrost.astype('int')
