...
import ETOCalc_parvec as ETOCalc 
import LGPCalc_parvec as LGPCalc
...
class ClimateRegime(object):
    ...
    def getLGP_parallel(self,nchunks):
        nx=self.meanT_daily.shape[1]
        xchunk=int(np.ceil(nx/nchunks))

        # chunk numpy arrays for computation
        tasklist=[]
        for c in range(nchunks):
            ...
            # make a dask task graph of delayed computations
            # arrays are return in dictionaries tagged with a chunk id 
            chunkdict=dask.delayed(LGPCalc.getLGP_chunk)(c, self.im_mask[:,xstart:xend],
                                                self.meanT_daily[:,xstart:xend,:],
                                                self.totalPrec_daily[:,xstart:xend,:],
                                                self.maxT_daily[:,xstart:xend,:],
                                                self.pet_daily[:,xstart:xend,:],
                                                self.lgpt5[:,xstart:xend])                                                
            tasklist.append(chunkdict)

        # execute all computations in parallel
        result=dask.compute(*tasklist)

        # collapse results into single dictionary
        resultdict={}
        for d in result:
            resultdict.update(d)

        # recombine chunks
        # first get a list of chunks in the correct order
        Etm365_chunks=[]
        Eta365_chunks=[]
        Wb365_chunks=[]
        Wx365_chunks=[]
        ...
        lgp_tot_chunks=[]
        for c in range(nchunks):
            Etm365_chunks.append(resultdict[c][0])
            Eta365_chunks.append(resultdict[c][1])
            Wb365_chunks.append(resultdict[c][2])
            Wx365_chunks.append(resultdict[c][3])
            Sb365_chunks.append(resultdict[c][4])
            kc365_chunks.append(resultdict[c][5])
            lgp_tot_chunks.append(resultdict[c][6])

        # now combine
        self.Etm365=np.concatenate(Etm365_chunks,axis=1)
        self.Eta365=np.concatenate(Eta365_chunks,axis=1)        
        self.Wb365=np.concatenate(Wb365_chunks,axis=1)        
        self.Wx365=np.concatenate(Wx365_chunks,axis=1)        
        self.Sb365=np.concatenate(Sb365_chunks,axis=1)        
        self.kc365=np.concatenate(kc365_chunks,axis=1)  
        lgp_tot=np.concatenate(lgp_tot_chunks,axis=1)
