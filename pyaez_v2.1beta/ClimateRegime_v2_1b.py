...
Jan 2023: Improved MultiCroppingZonation, AEZ Classification, LGP, LGPequv (Swun, KB)
"""

import numpy as np
import UtilitiesCalc_v2_1 as UtilitiesCalc
import ETOCalc_v2_1 as ETOCalc 
import LGPCalc_v2_1 as LGPCalc
...
class ClimateRegime(object):
    ...
    def setMonthlyClimateData(self, min_temp, max_temp, precipitation, short_rad, wind_speed, rel_humidity):
        ...
        # eliminate inf and nan in the results by replacing with zero        
        P_by_PET_daily=self.totalPrec_daily/self.pet_daily  # divide by 0 here yields inf
        zeromask=np.where(self.pet_daily==0,1,0)  # find where the zeros are
        P_by_PET_daily=np.where(zeromask==1,0,P_by_PET_daily) # replace the infs with 0
        self.P_by_PET_daily = np.nan_to_num(P_by_PET_daily) # replace the nans with 0 
    ...
    def setDailyClimateData(self, min_temp, max_temp, precipitation, short_rad, wind_speed, rel_humidity):
        ...
        self.meanT_daily = np.zeros((self.im_height, self.im_width, 365),dtype='float32')
        self.totalPrec_daily = np.zeros((self.im_height, self.im_width, 365),dtype='float32')
        self.pet_daily = np.zeros((self.im_height, self.im_width, 365),dtype='float32')
        ...
        for i_row in range(self.im_height):
            for i_col in range(self.im_width):
                ...
                self.pet_daily[i_row, i_col, :] = obj_eto.calculateETO()
#                 # when high lat grids don't get sun in winter, the arccos eq in ETOCalc yields nan, change to 0
#                 self.pet_daily[i_row, i_col, :] = np.where(~np.isfinite(self.pet_daily[i_row, i_col, :]),0,self.pet_daily[i_row, i_col, :])
    ...
    def getThermalClimate(self):
        ...
        for i_row in range(self.im_height):
            for i_col in range(self.im_width):
                ...
                # Seasonal parameters
                # summer_rain = np.sum(self.totalPrec_daily[i_row,i_col,90:273])
                # winter_rain = np.sum(self.totalPrec_daily[i_row, i_col, :90])+np.sum(self.totalPrec_daily[i_row, i_col,273::])
                # summer_et0 = np.sum(self.pet_daily[i_row, i_col, 90:273])
                # winter_et0 = np.sum(self.pet_daily[i_row, i_col, :90])+np.sum(self.pet_daily[i_row, i_col, 273::])
    ...
    def getLGP(self, Sa=100., D=1.):
        ...
        Sb_old = 0.
        Wb_old = 0.
#         Sb_old = np.zeros(clim_reg.im_mask.shape)
#         Wb_old = np.zeros(clim_reg.im_mask.shape)
        #============================
...
#         istart0save=np.empty(self.im_mask.shape)
#         istart1save=np.empty(self.im_mask.shape)
...
#             totalPrec_monthly, Ta365, lgpt5)
        ...
        for i_row in range(self.im_height):
            for i_col in range(self.im_width):
                ...
                # Prec365 = UtilitiesCalc.UtilitiesCalc().spread_month_year(totalPrec_monthly)
...
#                 istart0save[i_row,i_col]=istart0
#                 istart1save[i_row,i_col]=istart1


                for doy in range(0, 365):
                    p = LGPCalc.psh(0., self.Eto365[i_row, i_col, doy])
                    Eta_new, Etm_new, Wb_new, Wx_new, Sb_new, kc_new = LGPCalc.EtaCalc(
                        np.float64(Tx365[i_row, i_col, doy]), 
                        np.float64(Ta365[i_row, i_col, doy]),
                            # Ta365[i_row, i_col, doy]),
                        np.float64(Pcp365[i_row, i_col, doy]), 
                        Txsnm, 
                        Fsnm, 
                        np.float64(self.Eto365[i_row, i_col, doy]),
                        Wb_old, 
                        Sb_old, 
                        doy, 
                        istart0, 
        ...
        for i_row in range(self.im_height):
            for i_col in range(self.im_width):
                ...
                # pcp365X = np.append(self.totalPrec_daily[i_row, i_col, :], self.totalPrec_daily[i_row, i_col, :])
...
#         self.istart0=istart0save
#         self.istart1=istart1save
        if self.set_mask:
    ...
    def getMultiCroppingZones(self, t_climate, lgp, lgp_t5, lgp_t10, ts_t0, ts_t10):
        ...
        self.ts_g_t5=ts_g_t5
        self.ts_g_t10=ts_g_t10                       
                    ...
                    if self.im_mask[i_r, i_c] == self.nodata_val:
                        ...
                        if t_climate[i_r, i_c] == 1:
                            ...
                            if np.all([lgp[i_r, i_c] >= 360, lgp_t5[i_r, i_c] >= 360, lgp_t10[i_r, i_c] >= 360, ts_t0[i_r, i_c] >= 7200, ts_t10[i_r, i_c] >= 7000]) == True:
                                ...
                                multi_crop_rain[i_r, i_c] = 2  

                            else:
                                multi_crop_rain[i_r, i_c] = 1  
...
#         return [multi_crop_rain, multi_crop_irr]
        ...
        if self.set_mask:
            return [np.ma.masked_where(self.im_mask == 0, multi_crop_rain), 
                    np.ma.masked_where(self.im_mask == 0, multi_crop_irr)]
        else:
            return [multi_crop_rain, multi_crop_irr]                                                             
    ...
    # Class 6: Arctic Climate
            ...
            for i_c in range(self.im_width):
                if self.set_mask:
                    if self.im_mask[i_r, i_c] == self.nodata_val:
                        ...
                        # the output resulting from temperature amplitude logic consideration is not correct yet, so it's commented out
                        # ave_temp = np.copy(self.meanT_daily[i_r, i_c,:])
                        # meanT_monthly_v = obj_utilities.averageDailyToMonthly(ave_temp)
                        # temp_amplitude= np.max(meanT_monthly_v) - np.min(meanT_monthly_v)

                        # tropics highland
                        # if tclimate[i_r, i_c] == 1:
                        #     aez_tclimate[i_r, i_c] = 1
                        #     if temp_amplitude > 12.5:
        ...
        # mean_temp = np.copy(self.meanT_daily) # array is copied not to change the original calculated mean temperature
